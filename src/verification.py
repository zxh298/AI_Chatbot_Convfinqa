...
def build_retry_instruction(reason: str, question: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "If the selected evidence contains a plausible numeric candidate for this benchmark question, do not change the answer into a refusal; fix the formatting or calculation issue instead.",
            ...
            "If the system prompt asks for `Calculation plan JSON:`, include it again on retry.",
            "For direct number-selection questions, use a one-step `select` calculation plan rather than an empty-step plan.",
