"""Tests for evaluation answer normalization and comparison."""
...
class AnswerNormalizationTests(unittest.TestCase):
    ...
    def test_missing_percent_sign_is_not_inferred_from_unrelated_percent_input(self) -> None:
        prediction = "Final answer: 8444\nCalculation: 25% of total revenue represents the segment."
        question = "considering the amount and the percentage it represented, what was total revenue?"

        parsed = parse_answer_text(prediction, question=question)

        self.assertFalse(parsed.is_percent)
        self.assertAlmostEqual(parsed.value, 8444)

    def test_strict_match_falls_back_to_plain_value_when_executed_gold_uses_percentage_points(self) -> None:
        prediction = "Final answer: 18\nCalculation: the relevant ratings category was 18%."
        question = "what percentage of the total derivative receivable ratings were junk bonds?"

        self.assertTrue(answers_match(prediction, 18.0, question=question))
